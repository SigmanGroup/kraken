...
def RMSDPrune(conformers, atoms, cutoff):
    ...
    PrunedConformers = []
    PrunedConformersIndices = []

    for c in range(0, l):
        if not c in ToDel:
            PrunedConformers.append(conformers[c])
            PrunedConformersIndices.append(c)

    #return PrunedConformers
    free(RMSDMatrix)
    return PrunedConformers,PrunedConformersIndices
...
def StrictRMSDPrune(conformers, atoms, cutoff, ConfLimit):
    ...
    #return PrunedConformers
    free(RMSDMatrix)
    return PrunedConformers,PrunedConformersIndices, AdjCutoff
